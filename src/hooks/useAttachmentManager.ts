...
export function useAttachmentManager({ expense, userId, eventId }: UseAttachmentManagerProps) {
  ...
  const uploadAttachment = useCallback(async (file: File) => {
    if (!userId || !eventId || !expense) {
      throw new Error('User not authenticated, event not selected, or expense not loaded');
    }
  ...
  }, [userId, eventId, expense?.id, expense?.attachments, updateExpenseMutation, fileUploadState.actions]);

  // Delete attachment handler
  const deleteAttachment = useCallback(async (attachmentUrl: string) => {
    if (!userId || !eventId || !expense) {
      throw new Error('User not authenticated, event not selected, or expense not loaded');
    }
  ...
  const handlePrimaryFileChange = useCallback(async (file: File | null) => {
    if (file) {
      try {
  ...
  const handleEmptyStateFileChange = useCallback(async (file: File | null) => {
    if (file) {
      try {
        await uploadAttachment(file);
  ...
  const handleDeleteClick = useCallback((attachmentUrl: string) => {
