  ...
  try {
    ...
    for (const workspaceDoc of workspacesSnapshot.docs) {
      ...
      for (const eventDoc of eventsSnapshot.docs) {
        ...
        console.log(
          `   - totalBudgetedAmount: ${data.totalBudgetedAmount ?? 'MISSING'}`,
        );
        console.log(
          `   - totalScheduledAmount: ${data.totalScheduledAmount ?? 'MISSING'}`,
        );
        console.log(
          `   - totalSpentAmount: ${data.totalSpentAmount ?? 'MISSING'}`,
        );

        if (
          data.totalScheduledAmount === undefined ||
          data.totalScheduledAmount === null
        ) {
    ...
    if (eventsToFix.length > 0) {
      ...
      eventsToFix.forEach((event, index) => {
        console.log(
          `   ${index + 1}. ${event.name} (User: ${event.userId}, Event: ${event.eventId})`,
        );
      });
