 ...
 */
export function calculateEventStatus(totalBudgeted: number, totalSpent: number): EventStatus {
 ...
 */
export function calculateSpentPercentage(totalBudgeted: number, totalSpent: number): number {
 ...
 */
export async function updateEventTotals(userId: string, eventId: string): Promise<EventTotals> {
  ...
  try {
    ...
    const categoryTotals: Record<string, { budgeted: number; scheduled: number; spent: number }> = {};
    ...
    const spentPercentage = calculateSpentPercentage(totalBudgetedAmount, totalSpentAmount);
...
export async function addToEventTotals(
  ...
  if (amounts.budgeted === undefined && amounts.scheduled === undefined && amounts.spent === undefined) {
  ...
  try {
    ...
    const totalBudgetedAmount = (currentData.totalBudgetedAmount || 0) + (amounts.budgeted || 0);
    const totalScheduledAmount = (currentData.totalScheduledAmount || 0) + (amounts.scheduled || 0);
    const totalSpentAmount = (currentData.totalSpentAmount || 0) + (amounts.spent || 0);
...
export async function subtractFromEventTotals(
  ...
  try {
    ...
    const totalBudgetedAmount = Math.max(0, (currentData.totalBudgetedAmount || 0) - (amounts.budgeted || 0));
    const totalScheduledAmount = Math.max(0, (currentData.totalScheduledAmount || 0) - (amounts.scheduled || 0));
    const totalSpentAmount = Math.max(0, (currentData.totalSpentAmount || 0) - (amounts.spent || 0));
...
export async function updateEventTotalsComplex(
  ...
  try {
    ...
    if (changes.budgeted) {
      totalBudgetedAmount += (changes.budgeted.add || 0) - (changes.budgeted.subtract || 0);
      totalBudgetedAmount = Math.max(0, totalBudgetedAmount);
    }

    // Apply scheduled changes
    if (changes.scheduled) {
      totalScheduledAmount += (changes.scheduled.add || 0) - (changes.scheduled.subtract || 0);
      totalScheduledAmount = Math.max(0, totalScheduledAmount);
    }

    // Apply spent changes
    if (changes.spent) {
      totalSpentAmount += (changes.spent.add || 0) - (changes.spent.subtract || 0);
